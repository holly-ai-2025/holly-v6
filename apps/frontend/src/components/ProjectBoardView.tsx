...
import { getPhases, createPhase, updatePhase, getTasks, createTask, updateTask, updateBoard } from "../lib/api";
...
const ProjectBoardView: React.FC<ProjectBoardViewProps> = ({ board, onBoardDeleted }) => {
  ...
  const fetchPhases = async (boardId: number) => {
    try {
      ...
      const boardPhases = res.data.filter((p: Phase) => p.board_id === boardId && !p.archived);
  ...
  const fetchTasks = async (boardId: number) => {
    try {
      ...
      setTasks(res.data.filter((t: Task) => t.board_id === boardId && !t.archived));
    } catch (err) {
  ...
  const handleDialogSave = async (form: Partial<Task>) => {
    try {
      if (form.task_id) {
        ...
        await createTask({ ...form, board_id: board.board_id });
      }
      fetchTasks(board.board_id);
    } catch (err) {
  ...
  const handlePhaseSave = async (form: Partial<Phase>) => {
    try {
      await createPhase({ ...form, board_id: board.board_id });
      fetchPhases(board.board_id);
    } catch (err) {
  ...
  const handleBoardSave = async () => {
    try {
      await updateBoard(board.board_id, {
        name: editedName,
  ...
  return (
    <Paper sx={{ p: 3, borderRadius: 3, boxShadow: 2 }}>
      ...
      {/* Phases */}
      {phases.map((phase) => (
        <Box key={phase.phase_id} mt={4} ml={2} mr={2}>
          <Box
            display="flex"
            justifyContent="space-between"
            alignItems="center"
            onClick={() => setOpenGroups({ ...openGroups, [phase.phase_id]: !openGroups[phase.phase_id] })}
            sx={{ cursor: "pointer" }}
          >
            ...
            <Box>
              ...
              <Box display="flex" justifyContent="flex-start" mt={1}>
                <Button
                  ...
                  size="small"
                  sx={{ borderRadius: "8px" }}
                  onClick={() => handleAddTaskClick(phase.phase_id)}
                >
                  + Add Task
                </Button>
              </Box>
