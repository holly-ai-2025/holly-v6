...
async def ops_status():
    ...
    try:
        sha = subprocess.check_output(
            ["git", "rev-parse", "HEAD"], stderr=subprocess.STDOUT, cwd=ROOT_PATH
        ).decode().strip()
    except Exception as e:
        print(f"[ops_status] Git SHA lookup failed: {e}")
    return {
        "ok": True,
        "api": {"uptime": f"{uptime:.2f} seconds", "version": "1.0.0", "sha": sha},
...
async def health_check():
    ...
    try:
        ...
        sha = "unknown"
    return {"status": "healthy", "uptime": f"{uptime:.2f} seconds", "version": "1.0.0", "sha": sha}

@app.post("/ops/deploy")
async def ops_deploy():
    try:
        output = subprocess.check_output(["git", "pull"], stderr=subprocess.STDOUT, cwd=ROOT_PATH).decode()
...
# ===== File Operations (YAML compatibility) =====

@app.get("/ops/ls")
async def ops_ls(path: str = ROOT_PATH):
    try:
...
@app.post("/ops/exec")
async def ops_exec(request: Request):
    data = await request.json()
    cmd = data.get("cmd")
    if not cmd:
...
async def create_branch(request: Request):
    ...
    r = requests.get(f"https://api.github.com/repos/{GITHUB_REPO}/git/refs/heads/main", headers=headers)
    if r.status_code != 200:
        return {"ok": False, "step": "fetch-base", "error": r.text}
    sha = r.json()["object"]["sha"]
    payload = {"ref": f"refs/heads/{branch_name}", "sha": sha}
    r = requests.post(f"https://api.github.com/repos/{GITHUB_REPO}/git/refs", headers=headers, json=payload)
...
async def git_commit_multi(request: Request):
    ...
    r = requests.post(f"https://api.github.com/repos/{GITHUB_REPO}/git/trees", headers=headers, json=payload)
    if r.status_code not in (200, 201):
        return {"ok": False, "step": "create-tree", "error": r.text}
    new_tree = r.json()["sha"]

    payload = {"message": message, "tree": new_tree, "parents": [base_sha]}
    r = requests.post(f"https://api.github.com/repos/{GITHUB_REPO}/git/commits", headers=headers, json=payload)
    if r.status_code not in (200, 201):
        return {"ok": False, "step": "create-commit", "error": r.text}
    new_commit = r.json()["sha"]

    payload = {"sha": new_commit, "force": True}
    r = requests.patch(f"https://api.github.com/repos/{GITHUB_REPO}/git/refs/heads/{branch}", headers=headers, json=payload)
    if r.status_code != 200:
